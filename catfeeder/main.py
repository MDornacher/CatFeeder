...
def _main():
    ...
    while True:
        try:
            ...
            match, certainty = evaluate_image(frame, interpreter, input_details, output_details)
            ...
            if len(short_term_match_history) != CONSECUTIVE_MATCHES:
                logger.warning(f"Short term match history is broken ({short_term_match_history}) "
                               "and will be reset")
                short_term_match_history = [None] * CONSECUTIVE_MATCHES
                logger.debug(f"New short term match history: {short_term_match_history}")
                continue

            # check if feeding conditions are met
            if match in cats and len(set(short_term_match_history)) == 1 and feeder.is_open():
                if last_photo is None or (last_photo - datetime.datetime.now()).seconds > PHOTO_COOLDOWN:
